...
import '../dashboardCSS/AreaTabs.css';

function AreaTabs2({ areas, data, initialSelectedArea, initialSelectedSite }) {
      ...
      >
        {areas.map((area) => (
          <Tab
            ...
            value={area}
            label={area}
            className={`tab-item ${selectedArea === area ? 'active' : ''}`}
          />
        ))}
      </Tabs>
      {selectedArea && (
